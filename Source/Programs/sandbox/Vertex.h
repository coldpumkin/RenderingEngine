...
//
// Stride 48, all float so no padding. Ordered the way scene.vert reads them -- the
// object-space values first, then the one that is in no space at all:
//
//   position  12   object space. The push constant's model puts it in world
//   normal    12   object space. +z for a z=0 face wound CCW in y-up
//   tangent   16   object space, xyz along +u. w is a sign (glTF TANGENT)
//   uv         8   the surface's own coordinate. (0,0) top-left, y down
//
...
struct Vertex {
    ...
    float tangent[4];    // location 2
    float uv[2];         // location 3
};
