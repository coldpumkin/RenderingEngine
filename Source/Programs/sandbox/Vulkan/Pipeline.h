...
//
// Contract: every vertex stage built into these pipelines emits triangles with
//           positive shoelace area in clip space. Measured, not derived -- nothing
//           says so until a pipeline culls.
...
// this is a value with defaults rather than a list each caller names.
//
// The defaults are what drawing one flat thing wants: no culling, no depth.
struct RasterState {
    // One field, because the viewport's sign and the winding test have to agree.
    ViewportY viewportY = ViewportY::Down;

    // The value a draw inherits. A caller may change it per draw with one command.
...
// viewport covers is not. It is a rect and not an extent because a caller may draw into
// less than the whole target, and the bars are the difference.
//
...
//           a separate question, since there is no projection to agree with.
//
...
//
// Contract: whoever binds a Translucent pipeline leaves depthWrite off -- a surface
//           mixed with what is behind it must not hide what comes after. Depth write is
...
// Everything one pipeline is created from. A struct rather than arguments because the
// fields are settled at different times and by different owners, and a pipeline is
// defined by all of them together.
//
// The sample count is not among them. It sits inside AttachmentFormats, projected from
// targets -- a field of its own would make "the attachment is 4x and the pipeline is
// 1x" expressible.
struct GraphicsPipelineDesc {

    // Borrowed. Two pipelines differing only in baked state share one, which is what
    // makes them two pipelines and not two programs.
